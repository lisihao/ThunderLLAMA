...
echo ""
echo "🔥 启动优化配置 (移除 KV cache 量化)"
echo "  - Paged Attention: ✅"
echo "  - Flash Attention: on"
echo "  - KV Cache: f16 (默认，无量化)"
...
./build/bin/llama-server \
  ...
  --host 0.0.0.0 \
  --port 8090 \
  > /tmp/optimized-no-quant-30b.log 2>&1 &

SERVER_PID=$!
echo "🚀 Server PID: $SERVER_PID"
echo "📝 Logs: /tmp/optimized-no-quant-30b.log"
echo ""
echo "等待服务器启动..."
...
echo ""
echo "❌ 服务器启动超时，检查日志: /tmp/optimized-no-quant-30b.log"
exit 1
