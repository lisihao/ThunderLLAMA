#!/usr/bin/env python3
"""30B 模型端到端性能测试 - 优化版（移除 KV cache 量化）"""
...
def main():
    ...
    # 汇总
    print(f"\n{'='*60}")
    print("📊 Optimized (No KV Quant) 性能汇总 (30B 模型)")
    print(f"{'='*60}")
    ...
    # 保存
    results = {
        "config": "optimized_no_kv_quant",
        "model": "Qwen3-30B-A3B-128K-Q5_K_M",
        "endpoint": "completion",
        "optimizations": {
            "paged_attention": True,
            "flash_attention": "on",
            "kv_cache_type": "f16",  # 无量化
    ...
    output_file = "/tmp/thunderllama-optimized-no-quant-30b.json"
